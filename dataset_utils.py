...
def image_to_tfexample(image_data, image_format, height, width, class_id):
    return tf.train.Example(features=tf.train.Features(feature={
            ...
            'image/format': bytes_feature(image_format.encode('utf-8')),
...
def _get_dataset_filename(output_dir, split_name, shard_id, tfrecord_filename, _NUM_SHARDS):
    output_filename = '%s_%s_%05d-of-%05d.tfrecord' % (tfrecord_filename, split_name, shard_id, _NUM_SHARDS)
    return os.path.join(output_dir, output_filename)
    

def _convert_dataset(split_name, filenames, class_names_to_ids, dataset_dir, output_dir, tfrecord_filename, _NUM_SHARDS):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session('') as sess:
            ...
            for shard_id in range(_NUM_SHARDS):
                ...
                output_filename = _get_dataset_filename(output_dir, split_name, shard_id+1, tfrecord_filename = tfrecord_filename, _NUM_SHARDS = _NUM_SHARDS)
                print("Processing TFRecord File:", output_filename)
...
def _dataset_exists(dataset_dir, _NUM_SHARDS, output_filename):
    for split_name in ['train', 'validation']:
        for shard_id in range(_NUM_SHARDS):
            tfrecord_filename = _get_dataset_filename(dataset_dir, split_name, shard_id, output_filename, _NUM_SHARDS)
